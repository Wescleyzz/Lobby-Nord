package br.saintmc.lobby.inventory.kitpvp;
...
public class KitPvPInventory {
    ...
    public KitPvPInventory(Player player) {
        ...
        serverList.addAll(BukkitMain.getPlugin().getServerManager().getBalancer(ServerType.FULLIRON).getList());
        serverList.addAll(BukkitMain.getPlugin().getServerManager().getBalancer(ServerType.SIMULATOR).getList());

        MenuInventory menu = new MenuInventory("§7Servidores de KitPvP", 2 + (serverList.size() == 0 ? 1 : (serverList.size() / 7) + 1));
    ...
    public void create(List<SaintServer> serverList, Member member, MenuInventory menu) {
        ...
        for (SaintServer server : serverList) {
            ...
            builder.lore("\n§7Servidor do modo §b§l" + (server.getServerType() == ServerType.FULLIRON ? "FULL-IRON" : "SIMULATOR"));
